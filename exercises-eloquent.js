 ...
 *  generation in the data came out. Let’s take a closer look at that phenomenon.
 *
 *  Compute and output the average age of the people in the ancestry data set per century. A person is
 *  assigned to a century by taking their year of death, dividing it by 100, and rounding it up, as in
 *  Math.ceil(person.died / 100).
 *
 *  For bonus points, write a function groupBy that abstracts the grouping operation. It should accept
 *  as arguments an array and a function that computes the group for an element in the array and returns
...
(function(){
  ...
  /**
   *  Get average value for a set of numbers.
   * 
   *  @param {Number[]} array - Input array.
   *
   *  @returns {Number} - Average value.
 ...
 *  when called with an array element as argument, returns true or false. Just like && returns a true
 *  value only when the expressions on both sides are true, every returns true only when the predicate
 *  returns true for all elements of the array. Similarly, some returns true as soon as the predicate
 *  returns true for any of the elements. They do not process more elements than necessary—for example,
...
(function() {
   ...
   *  @param {Function} fn - Function to test for each element.
   *
   *  @returns {Boolean} - `true` if the callback function returns a truthy value for any array element;
   *                        otherwise, `false`.
   */
  const some = (arr, fn) => {
    for (let i = 0; i < arr.length; i++) {
 ...
 *  a new vector that has the sum or difference of the two vectors’ (the one in this and the parameter) x and
 *  y values.
 *
